...
function [a, b, alpha, w, res] = gamma_linear_regress( X, Y , init_param, opt_w, init_w)

%%=============================================================
%% Project:   L2E
%% Module:    $RCSfile: linear_regression_gamma.m,v $
%% Language:  MATLAB
%% Author:    $Author: bjian $
...
Lb = [-Inf; -Inf];
Ub = [Inf; Inf];
Lb(p+2) = exp(-5);  % alpha >0 %% this constraint on sigma is very important !!
%Ub(p+2) = 5;
[final_param, min_value] = fmincon(@gamma_linear_regress_criterion, init_param, [],[], [],[], Lb, Ub, [], options, X, Y, opt_w, init_w);

a = final_param(1);
b = final_param(2:p+1);
alpha = final_param(p+2);
%beta = final_param(p+3);
if opt_w
  w = final_param(p+3);
else
  w = init_w;
